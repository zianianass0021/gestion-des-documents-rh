...
#[AsCommand(
    name: 'app:insert-2000-more-employees',
    description: 'Insert 2000 additional employees using optimized SQL approach',
)]
class Insert2000MoreEmployeesCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $io->title('Insertion de 2000 Employés Supplémentaires');
        ...
        $placardNames = [
            'Archives RH-2', 'Dossiers Financiers-2', 'Ressources Humaines-2',
            'Services Techniques-2', 'Direction Générale-2', 'Comptabilité-2',
            'Informatique-2', 'Marketing-2', 'Ventes-2', 'Production-2',
            'Archives Secrétariat-2', 'Dossiers Juridiques-2', 'RH Temporaires-2',
            'Archives Anciennes-2', 'Dossiers Spéciaux-2', 'Archives Digitales-2'
        ];
        ...
        for ($i = 0; $i < 2000; $i++) {
            ...
            $employees[] = [
                'prenom' => $prenom,
                'nom' => $nom,
                'email' => strtolower($prenom . '.' . $nom . '.' . rand(1000, 9999) . '@uiass.ma'),
                'username' => strtolower($prenom . $nom . rand(1000, 9999)),
        ...
        $contractTypes = $connection->executeQuery("SELECT id FROM p_nature_contrat")->fetchFirstColumn();
        $organizations = $connection->executeQuery("SELECT id FROM p_organisation")->fetchFirstColumn();
        ...
        foreach ($contractDistribution as $contractCount => $employeeCount) {
            for ($i = 0; $i < $employeeCount && $employeeIndex < 2000; $i++) {
                ...
                for ($j = 0; $j < $contractCount; $j++) {
                    ...
                    // Insérer le contrat
                    $contractSql = "INSERT INTO t_employee_contrat (id, employe_id, nature_contrat_id, date_debut, date_fin, statut, salaire) VALUES (nextval('t_employee_contrat_id_seq'), ?, ?, CURRENT_DATE, CURRENT_DATE + INTERVAL '1 year', 'actif', ?)";
                    $connection->executeStatement($contractSql, [$employeeId, $contractTypeId, $salaire]);
                    
                    // Associer à l'organisation
                    $orgContractSql = "INSERT INTO t_organisation_employee_contrat (id, organisation_id, employee_contrat_id, date_debut, date_fin) VALUES (nextval('t_organisation_employee_contrat_id_seq'), ?, (SELECT id FROM t_employee_contrat WHERE employe_id = ? ORDER BY id DESC LIMIT 1), CURRENT_DATE, CURRENT_DATE + INTERVAL '1 year')";
