...
#[AsCommand(
    name: 'app:rebalance-distribution',
    description: 'Rebalance distribution of dossiers in placards and contracts in organizations',
)]
class RebalanceDistributionCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $io->title('Rééquilibrage de la Distribution');
        ...
        // Réassigner tous les dossiers de manière équitable
        $dossierSql = "
            UPDATE t_dossier 
            SET placard_id = (
                SELECT id FROM p_placards 
                ORDER BY id 
                LIMIT 1 OFFSET (ROW_NUMBER() OVER() % " . count($placards) . ")
            )
        ";
        ...
        // Réassigner tous les contrats de manière équitable
        $contractSql = "
            UPDATE t_organisation_employee_contrat 
            SET organisation_id = (
                SELECT id FROM p_organisation 
                ORDER BY id 
                LIMIT 1 OFFSET (ROW_NUMBER() OVER() % " . count($organizations) . ")
            )
        ";
